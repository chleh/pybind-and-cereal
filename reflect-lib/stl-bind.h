                                            ...
                                            std::string const& name)
    {
        return pybind11::bind_vector<T, Holder>(m, name)
            ...
            .def("__setstate__", [](T& v, pybind11::list& l) {
                ...
                for (auto& e : l)
                    v.emplace_back(
                        std::move(e).cast<typename T::value_type>());
            });
    }
};
...
public:
                                                 ...
                                                 std::string const& name)
    {
        ...
        cl.def("__iter__",
               [](Vector& v) {
                   return pybind11::make_iterator<pybind11::return_value_policy::reference_internal,
                                        It,
                                        It,
                                        T*>(It(v.begin()), It(v.end()));
               },
               pybind11::keep_alive<0, 1>() /* Essential: keep list alive while iterator
                                     exists */
               );
        ...
        cl.def("__getstate__", [](Vector const& v) {
            ...
            std::cout << "  setting state " << demangle(typeid(Vector).name())
                      << std::endl;
            for (auto& e : l) {
                // v.emplace_back(std::move(e).cast<typename Vector::value_type>());
